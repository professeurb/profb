...
Dans le programme précédent, dans le corps de la fonction `est_possible`, on parcourt à chaque fois l'intégralité des lignes, colonnes et blocs pour déterminer si une valeur donnée peut être inscrite dans une case donnée.
...
Dans la phase de résolution, on travaillera donc sur la liste `cases` plutôt que de chercher quelle est la case suivante. La fonction `essaye_case` devient alors&nbsp;:
